...
<body>
    ...
    <header2>
        <nav>
            <a href="materia_prima.php" class="active">Materias Primas</a>
    ...
    <main>
        ...
        <form method="get" style="margin-bottom: 2rem; display: flex; gap: 1rem; align-items: center;">
            ...
            <a href="materia_prima.php" style="margin-left:1rem; margin-bottom:1rem">Limpiar</a>
        </form>
        <?php
            ...
            if ($resultado && mysqli_num_rows($resultado) > 0) {
                ...
                while ($fila = mysqli_fetch_assoc($resultado)) {
                    ...
                    if (isset($_SESSION['Rol']) && $_SESSION['Rol'] === 'admin') {
                        echo '<td>
                                <a href="modificar_materia.php?id=' . urlencode($fila['ID']) . '" class="contrast">Editar</a>
                                <a href="eliminar_materia.php?id=' . urlencode($fila['ID']) . '" class="secondary" 
                                    onclick="return confirm(\'¿Seguro que deseas eliminar esta materia prima?\')" style="padding-left:20px">Eliminar</a>
                              </td>';
                    } else {
                ...
                echo '<p>No hay materias primas para mostrar.</p>';
            }

            // Mostrar sección para agregar materias primas solo si el usuario es admin
            if (isset($_SESSION['Rol']) && $_SESSION['Rol'] === 'admin') {
                echo '
                <section style="margin-top:3rem;">
                    ...
                    <form method="post" action="agregar.php" style="display: flex; gap: 1rem; flex-wrap: wrap; align-items: center;">
                        <input type="hidden" name="tabla" value="materiaprima">
